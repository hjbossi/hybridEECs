...
TripletResult getTriplet(PseudoJet p1, PseudoJet p2, PseudoJet p3, double Rlow, double Rhigh) {
    ...
    return { triplet, maxDistance, tripletType, true };
}
                      ...
                      for (int j=0; j<int(constit.size());j++)
                          {
                            ...
                            for (int s=j+1; s<int(constit.size()) ;s++)
                            {
                                 ...
                                 for( int m=s+1; m<int(constit.size()); m++){
                                        ...
                                        if (result.isValid) {
                                            ParticleTriplet triplet = result.triplet;
                                            double maxDistance = result.maxDistance;
                                            type = result.tripletType;
                                            double energy_weight = ComputeEEEC(triplet, jet_pt, power_wt);
                                            if(argu4.find(yeswake) != std::string::npos)
                                            {
                                                eeec_xy->Fill(triplet.myCoordinates.x, triplet.myCoordinates.y, jet_pt,energy_weight*wt);
                                                if(type == 0)
                                                {
                                                eeec_jjj_xy->Fill(triplet.myCoordinates.x, triplet.myCoordinates.y, jet_pt,energy_weight*wt);
                                                }
                                                else if(type == 1)
                                                {
                                                 eeec_jjw_xy->Fill(triplet.myCoordinates.x, triplet.myCoordinates.y, jet_pt,energy_weight*wt);   
                                                }
                                                else if(type == 2)
                                                {
                                                eeec_jww_xy->Fill(triplet.myCoordinates.x, triplet.myCoordinates.y, jet_pt,energy_weight*wt);  
                                                }
                                                else{
                                                 eeec_www_xy->Fill(triplet.myCoordinates.x, triplet.myCoordinates.y,jet_pt,energy_weight*wt); 
                                                }
                                            }
                                            else{
                                                eeec_xy->Fill(triplet.myCoordinates.x, triplet.myCoordinates.y, jet_pt,energy_weight*wt); 
